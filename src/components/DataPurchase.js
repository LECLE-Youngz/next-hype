...
import { buyNFTPrompt } from '../helpers/nft'
import { parsePrice } from '../libs/blockchain'

function DataPurchase({ id, name, promptPrice, ownerName, userId, setDataPurchasePopup }) {
		...
		<div className='fixed top-0 right-0 z-30 h-screen w-screen flex items-center justify-center bg-gray-900 bg-opacity-50 select-none'>
			<div className="flex items-center justify-center text-gray-500 md:w-8/12 lg:w-6/12 xl:w-5/12">
				<div className="bg-white shadow-xl w-full px-16 py-5 h-3/4">
					<h3 className="self-center text-4xl mt-4 mb-5 text-gray-900"># Purchase generation data</h3>
					<div class="container mx-auto">
						<p className='font-semibold ml-10 text-gray-500'>you are about to purchase the
							<span className='text-gray-500 font-semibold twinkle-text'> {name} </span>
						...
						<div className='grid grid-cols-3 gap-4 mt-10 mx-10'>
							...
							<p className='col-span-2 text-lg text-gray-700 font-semibold'>{id}</p>
							<p className='text-lg text-gray-500 font-semibold'>## nanme</p>
							<p className='col-span-2 text-lg text-gray-700 font-semibold'>{name}</p>
							...
							<p className='col-span-2 text-lg text-gray-700 font-semibold'>{parsePrice(promptPrice)} eth</p>
						</div>
						<div className='container mx-auto grid gap-2 mt-10 w-1/2'>
							<button className="group border py-4 px-8 border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white transition-colors ease-in-out duration-500 disabled:pointer-events-none" onClick={() => summit()} disabled={onSummit || onSuccess}>
								<div className="relative flex items-center space-x-4 justify-center">
									{
										!onSummit && !onSuccess ?
											<p className='block w-max text-gray-900 text-lg transition duration-300 group-hover:text-gray-100'>{onSuccess === null ? "Purchase" : "Failed!"}</p>
											:
											!onSuccess ?
					...
					<div className="py-10 space-y-2 text-gray-600 sm:-mb-8">
						<p className="text-xs">{'<!-- '} nft purchase
							{' '}<span className='underline underline-offset-2'>does not</span> include the generation data {' -->'}</p>
						<p className="text-xs">{'<!-- '} your charge will be
							{' '}<span className='underline underline-offset-2'>higher than</span> the price shown up due to the network fee {' -->'}</p>
					</div>
				</div>
			</div>
