...
function Login({ setLoginPopup, setGoogleData }) {
  ...
  const login = useGoogleLogin({
    onSuccess: async tokenResponse => {
      try {
        setLoggingIn(true)
        const response = await getGoogleToken({ code: tokenResponse.code })
        ...
        <div className="bg-white shadow-xl w-full px-16 py-5">
          ...
          <div class="container mx-auto">
            <div className='grid gap-2'>
              <button className="group border py-4 px-8 border-gray-900 text-gray-900 hover:bg-gray-900 hover:text-white transition-colors ease-in-out duration-500 disabled:pointer-events-none" onClick={() => login()} disabled={loggingIn}>
                <div className="relative flex items-center space-x-4 justify-center">
                  <img src="https://tailus.io/sources/blocks/social/preview/images/google.svg" className="absolute left-0 w-5" alt="google logo" />
                  {
                    loggingIn ?
                      ...
                      :
                      <span className="block w-max text-gray-900 text-lg transition duration-300 group-hover:text-gray-100">Continue with Google</span>
                  }
                </div>
              </button>
              {/* <button className="group h-12 px-6 border-2 border-gray-300 rounded-full transition duration-300 
 hover:border-gray-500 focus:bg-gray-50 active:bg-gray-100">
                <div className="relative flex items-center space-x-4 justify-center">
                  ...
                  </svg>
                  {
                    loggingIn ?
                      ...
                      :
                      <span className="block w-max font-semibold tracking-wide text-gray-700 text-sm transition duration-300 group-hover:text-gray-600 sm:text-base">Continue with GitHub</span>
                  }
                </div>
              </button> */}
            </div>
