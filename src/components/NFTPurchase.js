        ...
        <div className='fixed top-0 right-0 z-30 h-screen w-screen flex items-center justify-center bg-gray-900 bg-opacity-50 select-none'>
            <div className="flex items-center justify-center text-gray-500 md:w-8/12 lg:w-6/12 xl:w-4/12">
                <div className="bg-white shadow-xl w-full px-16 py-5">
                    <h3 className="font-extrabold text-4xl text-gray-800 text-center mt-4 mb-10">Purchase confirmation</h3>
                    <div class="container mx-auto">
                        ...
                        <div className='grid grid-cols-3 gap-4 mt-10 w-3/4 mx-auto'>
                            ...
                            <p className='col-span-2 text-lg font-semibold'>{name}</p>
                            <p className='text-lg text-gray-700 font-semibold'>Transfer to</p>
                            <p className='col-span-2 text-lg font-semibold'>{ownerName}</p>
                            <p className='text-lg text-gray-700 font-semibold'>NFT price</p>
                            <p className='col-span-2 text-lg font-semibold'>{price} BTC</p>
                        </div>
                        <div className='flex'>
                            <button className={`${onSummit || onSuccess ? "border-gray-500 cursor-default" : "hover:border-gray-500"} group h-12 px-6 mt-10 mx-auto border-2 border-gray-200 rounded-full transition duration-300 w-1/3 disabled:cursor-default disabled:pointer-events-none`} onClick={() => summit()} disabled={onSummit || onSuccess}>
                                <div className="relative flex items-center space-x-4 justify-center">
                                    <span className="block font-semibold tracking-wide text-gray-700 text-sm transition duration-300 group-hover:text-gray-500 sm:text-base">
                                        {
                                            !onSummit && !onSuccess ?
                    ...
                    <div className="py-10 space-y-2 text-gray-600 text-center sm:-mb-8">
                        <p className="text-xs">NFT purchase
                            {' '}<span className='font-bold'>does include</span> the generation data.</p>
            ...
            <div className='h-screen w-screen absolute -z-10' onClick={() => !onSummit && setNFTPurchasePopup(false)}></div>
        </div >
    )
}
