...
function Transfer({ nfts, setTransferPopup }) {
    ...
    const summit = async () => {
        ...
        const res = await transferToAddress(nfts.map(nft => nft.nftId), userWallet.address)
        ...
        <div className='fixed top-0 right-0 z-30 h-screen w-screen flex items-center justify-center bg-gray-900 bg-opacity-50 select-none'>
            <div className="flex items-center justify-center text-gray-500 md:w-8/12 lg:w-6/12 xl:w-4/12">
                <div className="bg-white shadow-xl w-full px-16 py-5">
                    <h3 className="font-extrabold text-4xl text-gray-800 text-center mt-4 mb-10">Transfer NFTs</h3>
                        ...
                        <div className="grid space-y-5">
                            ...
                            <div className='flex justify-between items-center'>
                                ...
                                <button className={`${inputClass[isRootStock]} rounded-full border-2 cursor-pointer rounded-l-none h-12 font-semibold w-1/2 flex items-center justify-center`} defaultValue={!isRootStock}>
                                    Transfer at Ordinals
                                </button>
                            </div>
                        </div>
                        <div className='flex'>
                            <button className={`${onSummit ? "border-gray-500 cursor-default" : "hover:border-gray-500"} group h-12 px-6 mt-10 mx-auto border-2 border-gray-200 rounded-full transition duration-300 w-1/3 disabled:cursor-default disabled:pointer-events-none`} onClick={() => summit()} disabled={email === "" || userWallet === null || onSummit === onSuccess}>
                    ...
                    <div className="py-10 space-y-2 text-gray-600 text-center sm:-mb-8">
                        <p className="text-xs">All NFTs will be {' '}
                            ...
                            {' '}
                            another account on this service.
                        </p>
                        <p className="text-xs">They must have same chain unless you will have to bridge them.</p>
                    </div>
                </div>
            </div >
            <div className='h-screen w-screen absolute -z-10' onClick={() => !onSummit && setTransferPopup(false)}></div>
        </div >
    )
}
