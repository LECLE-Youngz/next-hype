		...
		<div className="flex flex-col justify-between">
					...
					<button
						...
						onClick={() =>
							navigate("/social/create", {
								state: { postId: postId, post: parsePost(post) },
							})
						}
					>
						Edit post
			...
			<div className="flex justify-between gap-1 mx-10 mt-10 mb-8">
				...
				<p className="truncate border-b w-1/2 border-gray-900 text-2xl font-extralight text-right flex">
					<p className="text-center appearance-none rounded-full flex-1 outline-none text-gray-600 w-full">
						## post view
					</p>
				</p>
				<p className="truncate w-min border-gray-900 text-2xl font-extralight">
				...
				<p className="truncate border-b w-1/4 border-gray-900 text-2xl font-extralight text-right flex">
					<p className="text-right appearance-none rounded-full flex-1 outline-none text-gray-600 w-full">
						## post detail
					</p>
				</p>
			</div>
				...
				<div className="flex col-span-3">
					<div className="grid grid-cols-3 gap-5 w-full">
						<div className="col-span-2">
							<p className="text-4xl text-gray-900 font-light font-serif">
								{post.header}
							</p>
							<p className="my-5 text-lg text-gray-600 font-semibold">
								{post.description}
							</p>
							...
							<div className="flex gap-2 mt-5 justify-end">
								{post.tags.map((tag) => (
									<Link
										to={`/social/tag/${tag}`}
										className="text-gray-600 font-light underline-offset-2 hover:text-black text-lg text-right hover:underline"
									>
										#{tag}
									</Link>
								))}
							...
							<div className="flex justify-between text-2xl text-gray-600">
								{!likeUpdating ? (
									post.likes.includes(account.id) ? (
										<div
											className="w-10 flex items-center space-x-1 group cursor-pointer hover:text-red-400 text-red-600"
											onClick={() => toggleLike()}
										>
										...
										<div
											className="w-10 flex items-center space-x-1 group cursor-pointer hover:text-red-400"
											onClick={() => toggleLike()}
										>
											<BiUpvote className="" />
											<p className="text-lg">{post.likes.length}</p>
								...
								<a
									className="flex items-center space-x-1 group cursor-pointer hover:text-gray-400"
									href="#comment"
								>
									<BiComment className="" />
								...
								{!bookmarkUpdating ? (
									post.bookmark.includes(account.id) ? (
										<div
											className="flex w-10 items-center space-x-1 group cursor-pointer hover:text-blue-400 text-blue-600"
										...
										<div
											className="flex w-10 items-center space-x-1 group cursor-pointer hover:text-blue-400"
											onClick={() => toggleBookmark()}
										>
							...
							<div className="grid pt-3">
								{post.listComment.map((comment, index) => (
									<div className="">
											...
											<div className="flex justify-end space-x-3">
												{likeCommentLoading !== comment.id ? (
													comment.likes.includes(account.id) ? (
														<>
															<div
																className="flex items-center space-x-1 group cursor-pointer hover:text-red-400 text-red-600"
														...
														<div
															className="flex items-center space-x-1 group cursor-pointer hover:text-red-400"
															onClick={() => toggleLike(comment.id)}
														>
															<BiUpvote className="" />
												...
												{replies[index] === null ? (
													...
													>
														...
														<p className="text-sm">
															{comment.listReplyComment.length}
														</p>
													</div>
												) : (
													<div
										...
										<div
											className={`${
												showReplies[index] ? "block" : "hidden"
											} pl-10 container bg-gray-100 p-3 transition-all duration-500 ease-in-out space-y-5`}
										>
										...
										{replies[index] !== null && (
											<div className="relative bg-gray-100 p-3 pl-10">
												<textarea
													...
													className="w-full h-24 p-3 border cursor-text focus:outline-black flex items-center justify-center"
													type="text"
												...
												{onReply === comment.id ? (
													...
													<div className="absolute bottom-6 right-6">
														<IoSend
															className={`${
																replies[index] !== "" ? "" : "hidden"
															} w-5 h-5 cursor-pointer text-gray-400 hover:text-gray-800`}
															onClick={() => send(index, comment.id)}
														>
															post
														</IoSend>
													</div>
										...
										<div
											div
											className="my-5 border-[0.5px] border-gray-200"
										></div>
										{comment.listReplyComment.length > 0 && (
											<div className="w-full flex justify-center">
												<p
													className="-translate-y-8 bg-white px-3 text-gray-600 hover:text-gray-900 hover:font-medium cursor-pointer"
													onClick={() => handleShow(index)}
												>
													{showReplies[index] ? "hide" : "show"}{" "}
													{comment.listReplyComment.length} replies
												</p>
											</div>
										)}
									</div>
									...
									{onComment ? (
										...
										<div className="absolute bottom-3 right-3">
											<IoSend
												className={`${
													postComment !== "" ? "" : "hidden"
												} w-6 h-6 cursor-pointer text-gray-400 hover:text-gray-800`}
												onClick={() => send()}
											></IoSend>
										</div>
									)}
