...
export default function Generate() {
	...
	const [generateParams, setGenerateParams] = useState(
		state?.meta ?? {
			prompt: "",
			negative_prompt:
				"(worst quality, low quality, large head, extra digits:1.4) ",
			width: 512,
			height: 512,
			samples: 1,
	...
	const generate = async () => {
		...
		if (res.status === "processing") {
			...
			while (fetch.status === "processing") {
				...
				fetch = await fetchImage(res.id);
			}
			console.log(res, fetch);
			res.output = fetch.output;
	...
	return (
		<div className="flex flex-col justify-between">
			...
			<h1 className="text-4xl text-gray-900">
				# Turn your <span className="twinkle-text">imagination</span> into NFTs
			</h1>
			...
			<div className="text-gray-900 mx-5 font-light grid grid-cols-2">
				<div className="grid items-center mx-auto w-5/6">
					<div className="gap-y-2 mx-auto w-full z-20">
						...
						<textarea
							onChange={(e) =>
								setGenerateParams({ ...generateParams, prompt: e.target.value })
							}
							className={`${
								inputClass[generateParams.prompt === ""]
							} w-full h-24 p-3 border cursor-text focus:outline-black flex items-center justify-center `}
							type="text"
							defaultValue={generateParams.prompt}
						/>
						...
						<textarea
							...
							className={`${
								inputClass[generateParams.negative_prompt === ""]
							} w-full h-24 p-3 border cursor-text focus:outline-black flex items-center justify-center `}
							type="text"
							defaultValue={generateParams.negative_prompt}
						/>
							...
							<div className="w-1/2">
								...
								<div className="w-full flex gap-4 items-center justify-between group">
									<div
										className={`${
											imageSizeClass[
												generateParams.height === 768 &&
													generateParams.width === 512
											] + " w-16 h-24 border flex items-center cursor-pointer"
										}`}
										onClick={() =>
											setGenerateParams({
												...generateParams,
									...
									<div
										className={`${
											imageSizeClass[
												generateParams.height === 512 &&
													generateParams.width === 512
											] + " w-16 h-16 border flex items-center cursor-pointer"
										}`}
										onClick={() =>
											setGenerateParams({
												...generateParams,
									...
									<div
										className={`${
											imageSizeClass[
												generateParams.height === 512 &&
													generateParams.width === 768
											] + " w-24 h-16 border flex items-center cursor-pointer"
										}`}
										onClick={() =>
											setGenerateParams({
												...generateParams,
								...
								<div className="w-full flex">
									<input
										...
										defaultValue={generateParams.num_inference_steps}
									...
									<label
										for="customRange1"
										className="self-center text-base text-gray-900 w-1/12 text-right"
									>
										{generateParams.num_inference_steps}
									</label>
								</div>
								...
								<div className="w-full flex">
									<input
										...
										defaultValue={generateParams.guidance_scale}
										className="self-center transparent h-1 w-full cursor-pointer appearance-none border-transparent bg-gray-200 accent-gray-900 hover:accent-gray-500"
										id="customRange1"
									/>
									<label
										...
										className="self-center text-base text-gray-900 w-1/12 text-right"
									>
										{Number(generateParams.guidance_scale).toFixed(1)}
									</label>
								</div>
							</div>
							...
							<div className="grid w-1/2">
								...
								<div className="w-full flex">
									<button
										...
										className={`${
											inputClass[generateParams.safety_checker !== "yes"]
									...
									<button
										...
										className={`${
											inputClass[generateParams.safety_checker !== "no"]
										} border cursor-pointer none h-12 w-1/2 flex items-center justify-center`}
									>
								...
								<div className="w-full flex">
									<button
										...
										className={`${
											inputClass[generateParams.enhance_prompt !== "yes"]
										}  border cursor-pointer none h-12 w-1/2 flex items-center justify-center`}
									>
										<p className="text-center w-full">yes</p>
									</button>
									<button
										...
										className={`${
											inputClass[generateParams.enhance_prompt !== "no"]
										}  border cursor-pointer none h-12 w-1/2 flex items-center justify-center`}
									>
										<p className="text-center w-full">no</p>
									</button>
				...
				<div className="flex flex-col items-center mt-10">
					<div
						className={`${previewSizeClass(
							generateParams.height / generateParams.width
						)} ${
							inputClass[typeof response !== "object" || response == null]
						}  border-gray-400  border-[1.5px] border-dashed grid self-center`}
					>
						{typeof response === "object" && response !== null ? (
							<div className="relative">
								<img
									src={response?.output?.[0]}
									style={{ width: "100%", height: "100%", objectFit: "cover" }}
									className="xl"
								/>
								<div
									className={`${
										onGenerate ? "block" : "hidden"
					...
					<div className="mt-10">
						<button
							className={`group h-12 w-40 mx-auto border border-gray-900 hover:bg-gray-900 transition duration-300 disabled:cursor-default disabled:pointer-events-none`}
							onClick={() => setMintPopup(true)}
							disabled={typeof response !== "object" || response == null}
						>
							<div className="relative flex items-center space-x-4 justify-center">
								<span className="block text-gray-900 text-sm transition duration-300 group-hover:text-gray-200 sm:text-base">
			...
			{typeof response === "object" && response !== null && (
				<div className="w-full mx-5">
					<h3 className="mt-10 mb-5 text-2xl text-gray-900 mx-5 font-extralight">
						## generation details
					</h3>
					<div className=" mx-14 border border-dashed border-gray-200 bg-gray-100 p-5 ">
						{
							<div className="grid grid-cols-6 gap-x-10 gap-y-5 items-center">
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.generationTime)}
								>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.prompt)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.negative_prompt)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.model)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.vae)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() =>
										copyText(response.meta.W + "x" + response.meta.H)
									}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.steps)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.guidance_scale)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
								...
								<p
									className="group relative font-medium col-span-5 hover:bg-gray-200 hover:cursor-pointer py-2 px-4 hover:text-gray-800 w-full"
									onClick={() => copyText(response.meta.seed)}
								>
									<span className="pointer-events-none absolute right-4 bottom-2 opacity-0  transition-opacity group-hover:opacity-100">
										{copy}
									</span>
